...
export default function AccountClient() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" className="w-full flex items-center space-x-3">
            <Avatar>
              {ensAvatar ? <AvatarImage src={ensAvatar} alt="ENS Avatar" /> : <AvatarFallback>?</AvatarFallback>}
            </Avatar>
            <div className="text-left">
              <div className="font-medium">{ensName || shortenString(address || "", 8)}</div>
              <div className="text-sm text-gray-500">{shortenString(address || "", 8)}</div>
            </div>
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-80 p-4 bg-white shadow rounded-lg">
          <div className="space-y-4">
            <div>
              ...
              <div className="text-sm text-gray-600">{ensName || "Not available"}</div>
            </div>
            <Separator />
            <div>
              <Label className="text-base text-gray-800">Wallet Address:</Label>
              <div className="text-sm text-gray-600 font-mono break-words">{address || "N/A"}</div>
            </div>
